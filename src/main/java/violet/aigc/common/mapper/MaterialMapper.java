...
public interface MaterialMapper {
    @Insert("INSERT INTO material VALUES (null,#{materialId},#{materialType},#{userId},#{prompt},#{sourceUrl},#{materialUrl},#{coverUrl},#{model},#{createTime},#{modifyTime},#{status},#{extra})")
    Boolean insertMaterial(Material material);

    @Update("UPDATE material SET material_url = #{materialUrl}, cover_url = #{coverUrl}, status = #{status}, modify_time = #{modifyTime} WHERE material_id = #{materialId}")
    ...
    @Select("SELECT * FROM material WHERE user_id = #{userId} AND status != 4 ORDER BY modify_time DESC LIMIT #{size} OFFSET #{offset}")
